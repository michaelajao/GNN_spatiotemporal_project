...
- Distribution, Cumulative, and Heatmap of Prediction Errors
- Scatter Plots of Actual vs. Predicted
- Summary bar charts comparing overall metrics (MAE, MSE, R², PCC) across experiments
...
class RegionAwareConv(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.activate(x).permute(0, 2, 1)
...
class EpiGNN(nn.Module):
    ...
    def forward(self, X, adj, adjacency_type='hybrid', states=None, dynamic_adj=None, index=None):
        ...
        d = torch.sum(adj, dim=1).unsqueeze(2)
        ...
        if adjacency_type == 'static':
            ...
            raise ValueError("Invalid adjacency_type: choose 'static', 'dynamic', or 'hybrid'.")
...
def run_experiment(adjacency_type='hybrid', experiment_id=1, summary_metrics=[]):
    ...
    'static', 'dynamic', or 'hybrid'.
    experiment_id: integer ID (e.g., 1, 2, 3)
    summary_metrics: list to accumulate results across experiments
    """
    ...
    for epoch in range(num_epochs):
        ...
        for batch_X, batch_Y in train_loader:
            ...
            if adjacency_type == 'static':
                ...
                raise ValueError("Invalid adjacency_type. Use 'static', 'dynamic', or 'hybrid'.")
    ...
    def plot_learned_adjacency(model, adj_static, regions, adjacency_type, experiment_id):
        ...
        single_sample_X = example_X[0].unsqueeze(0)

        with torch.no_grad():
            X_reshaped = single_sample_X.permute(0, 3, 1, 2)
            emb_for_adj = model.backbone(X_reshaped)  # => (1, m, hidR)
            learned_adj = model.graphGen(emb_for_adj)
            learned_adj = learned_adj.squeeze(0).cpu().numpy()
            ...
            if adjacency_type == 'static':
                ...
                d_mat = np.sum(adj_static.cpu().numpy(), axis=1, keepdims=True) * \
                        np.sum(adj_static.cpu().numpy(), axis=0, keepdims=True)
                gate_mat = model.d_gate.cpu().numpy()
                d_mat = 1 / (1 + np.exp(-gate_mat * d_mat))
...
if __name__ == "__main__":
    ...
    def plot_summary_metrics(summary_pivot):
        ...
        for i, metric in enumerate(metrics_list):
            plt.subplot(2, 2, i+1)
            sns.barplot(x='Adjacency_Type', y=metric, data=summary_pivot, hue='Adjacency_Type', palette='Set2', legend=False)
            plt.title(f'Average {metric}')
            plt.ylabel(metric)
            plt.xlabel('Adjacency Type')
            # plt.ylim(0, max(summary_pivot[metric]) * 1.2)
            for index, row in summary_pivot.iterrows():
                plt.text(index % 3, row[metric] + 0.01*row[metric], f"{row[metric]:.4f}",
